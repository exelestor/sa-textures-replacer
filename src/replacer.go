...
func main() {
	...
	files, err := filepath.Glob("*.txd")
	check(err)

	for _, fa := range files {
		fmt.Print("Working with '" + fa + "'... ")
		...
		n, err := os.OpenFile(fa, os.O_RDWR, 0755)
		check(err)
		for i := 0; uint16(i) < txd.Info.Count; i++ {
			if txd.Textures[i].Data.Width > 32 && txd.Textures[i].Data.Height > 32 &&
				txd.Textures[i].Data.Width < 2048 && txd.Textures[i].Data.Height < 2048 &&
				txd.Textures[i].Data.TextureFormat == "DXT1" {

				path := "dds/" + strconv.Itoa(int(txd.Textures[i].Data.Width)) + "x" + strconv.Itoa(int(txd.Textures[i].Data.Height)) + ".dds"
				if debug {
					fmt.Println()
					fmt.Print(path + ": ")
				}
				...
				bytesf = bytesf[0x80:]
				o.Close()
				written, err := n.WriteAt(bytesf, int64(txd.Textures[i].Data._DataStart))
				if debug {
					fmt.Println(written)
				}
				check(err)
